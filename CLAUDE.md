...
Externally developed themes, hosted on GitHub, built against `docs/THEME_API.md` and deployed to the operator's own Coolify. How it works: `docs/theme-deployments.md`; wire contract: `docs/platform-control-api.md` §9–12.
...
- The Caddy map in `theme-routes.caddy` comes from **one** table, `buildRoutingTable` in `src/deploy/routing.ts`: `live` + `edge` rows of `active` sites whose primary domain is verified and still equals the row's `domain`, plus verified aliases. `GET /api/platform/routing` serves it to `scripts/render-theme-routes.mjs`; with `THEME_ROUTES_FILE` set the web process renders it itself (shared core `src/lib/deploy/theme-routes.mjs`). A transition *requests* regeneration (`requestThemeRoutesRegeneration` — coalesced, read after commit, never fails its caller) and the queue re-renders once a minute as a safety net. Every write is atomic and a failure leaves the previous map; a truncated map takes every themed site offline at once. The file is **committed even though it is generated and empty by default** — a missing `import` is a Caddy that will not boot. In `docker-compose.prod.yml` it lives in the `theme_routes` **directory** volume that Caddy mounts read-only and `--watch`es: a single-file bind mount never sees a rename.
...
- Deploys run in the **jobs queue** (`advanceDeployments`, the first real task in `jobs.tasks`), never in the request: create, redeploy and rollback write a `queued` row and answer `202`; the queue (or `…/poll`) claims it `queued → creating` and runs `advanceDeployment`. Status moves through `canTransition` in `src/lib/deploy/status.ts` — illegal moves are logged and dropped, not written, so a late retry cannot overwrite a terminal state.
- **One Coolify application per (site, theme, production|preview)**, found again by its deterministic name. A reused application is re-pointed (`git_branch`/`git_commit_sha`/`domains`) before it builds, or a redeploy silently ships the old commit; a superseded row sharing the new row's `appUuid` is marked stopped *without* stopping the application. A `preview` has its own application and hostname (`…-preview`), only ever supersedes other previews, and never moves `renderedBy`.
- The health check and the revalidate URL use `applicationHostOf` (the preview hostname) — the customer domain may still point at Caddy, whose built-in renderer answers 200. `ESHOBE_PUBLIC_ORIGIN` is the row's `domain`: the customer domain in `edge`/`direct`.
- **Site lifecycle is a hook on `sites`** (`src/deploy/lifecycle.ts`), so every write path gets it: suspended/archived stops every application-holding row through `stopDeployment` (never delete, keep history, revoke keys; best-effort, failures written onto the row); reactivation restarts **nothing**. A primary-domain change leaves live `edge`/`direct` rows running but `needsRedeploy` (derived on read, never stored), dropped from routing, and unpromotable (`promotionBlocker`). `sites.renderedBy`/`activeDeployment` have `access: () => false` for every caller — the service writes them with `overrideAccess`, and a stale admin form must not put old values back.
- **Deployable themes are operator-managed.** There is no customer theme picker. The customer's surface is «تنظیمات پوسته»: `GET|POST /api/site-theme-settings/current` (collection endpoints; session of a site member, owner writes; package from the site's deployments; only tenant-declared keys; secrets write-only, blank = unchanged, explicit `clear`). The raw collection's create/update are platform-only, and `secretValues` is masked by an `afterRead` hook that only the deploy job's context flag bypasses — `encryptThemeSettings` re-reads the stored blob when the incoming value is blank or the mask.
- Update-available is a **commit comparison** (`updateInfoFor`): the running `commitSha` against the pin, else `theme-packages.syncedCommitSha` (written by sync, cleared when `defaultRef` is edited). No GitHub call on read. `effectiveRefFor` is the one place ref precedence lives (explicit › pin › default ref).
...
- The renderer webhook signs the **raw body only**, not `<timestamp>.<body>` like the platform webhooks — `docs/THEME_API.md` §17 is already published and deployed themes verify it that way. `x-eshobe-timestamp` is sent unsigned. Changing it would fail silently: renderers would reject every notice and serve stale pages. That is a v2 change, made in the doc first; `signRendererBody` is pinned against independently computed bytes in `tests/int/renderer-webhook.int.spec.ts`.
