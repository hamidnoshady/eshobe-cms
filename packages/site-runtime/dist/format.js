...
const intlLocale = (locale) => (locale === 'fa' ? 'fa-IR' : locale);
...
export const formatDate = (date, locale, options = { dateStyle: 'long' }) => {
...
export const formatNumber = (value, locale, options) => new Intl.NumberFormat(intlLocale(locale), options).format(value);
/**
 ...
 * stored value.
 */
export const formatPrice = (minor, code, locale, options = {}) => {
    ...
    if (options.hideUnit)
        return amount;
    const unit = currency.unit[locale] ?? currency.unit.en;
    return `${amount} ${unit}`;
};
