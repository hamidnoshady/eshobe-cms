 ...
 */
export const isHexColor = (value) => typeof value === 'string' && /^#[0-9a-f]{3}([0-9a-f]{3})?$/i.test(value);
/** The `--radius` scale `globals.css` derives `--radius-sm|md|lg|xl` from. */
const RADIUS = { lg: '1rem', md: '0.625rem', none: '0', sm: '0.25rem' };
...
const pair = (name, value) => isHexColor(value) ? `--${name}:${value};--${name}-foreground:${readableOn(value)};` : '';
/** A colour on its own — for the two tokens that already name their own contrast. */
const decl = (name, value) => isHexColor(value) ? `--${name}:${value};` : '';
export const themeCss = (theme) => {
    if (!theme)
        return '';
    const radius = RADIUS[theme.radius ?? 'md'] ?? RADIUS.md;
    const leading = typeof theme.lineHeight === 'number' &&
