 ...
 * admin's picker uses.
 */
const ALL = ['business', 'portfolio', 'store'];
const library = [
...
export const siteBlocks = library.map(({ slug }) => slug);
export const blockSlugsForSiteType = (type) => library.filter(({ siteTypes }) => !type || siteTypes.includes(type)).map(({ slug }) => slug);
