...
export const Payments: GlobalConfig = {
  ...
  admin: {
    ...
    /**
     * Platform provider *policy* — which PSPs the SaaS permits at all — so it is
     * platform infrastructure, not a customer's Commerce menu. A site's own
     * enable/configure surface is `payment-gateways` (فروشگاه → پرداخت): the
     * operator sets availability here; the customer switches theirs on there.
     */
    group: 'زیرساخت',
